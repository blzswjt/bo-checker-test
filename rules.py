...
包含：
  DA 数据架构：主题域分类、主题域分组、主题域、业务对象、逻辑实体、业务属性
  BA 业务架构：流程分类（L1）、流程组（L2）、业务流程（L3）、子流程（L4）、业务活动（L5）
...
ELEMENT_TYPES = [
    "主题域分类", "主题域分组", "主题域",
    "业务对象", "逻辑实体", "业务属性",
    "流程分类（L1）", "流程组（L2）", "业务流程（L3）", "子流程（L4）", "业务活动（L5）"
]

# 领域分组（前端分组展示用）
DOMAINS = {
    "📦 数据架构DA": ["主题域分类", "主题域分组", "主题域", "业务对象", "逻辑实体", "业务属性"],
    "🏗 业务架构BA": ["流程分类（L1）", "流程组（L2）", "业务流程（L3）", "子流程（L4）", "业务活动（L5）"],
}
...
ELEMENT_RULES = {
    ...
    # ============================================================
    # BA 业务架构：流程分类（L1）~ 业务活动（L5）
    # ============================================================
    "流程分类（L1）": {
        "description": "从客户价值出发的公司最高阶流程分类，体现公司的业务模式和价值链特点，覆盖公司全部的业务活动。通常分为运营类、使能类、支持类三大类别。",
        "identification": [
            {"rule": "价值导向", "desc": "L1流程类应与企业价值流对应或覆盖。运营类流程从客户价值出发，体现公司业务模式和价值链特点。",
             "positive": "“线索到回款”对应从营销获客到回款的完整价值流", "negative": "以内部部门职能划分的高阶分类（如“财务部流程”）不是L1流程类"},
            {"rule": "分类完整性", "desc": "覆盖企业所有核心业务活动，通常包括运营类、使能类、支持类三大类别。",
             "positive": "运营类：“市场到线索”“线索到回款”“概念到市场”；使能类：“服务交付”“采购”；支持类：“管理人力资源”“管理财经”", "negative": "只覆盖单个部门或单条价值链的分类不完整"},
        ],
        "naming": [
            {"rule": "运营类流程命名", "desc": "针对运营类流程：可与价值流对齐，可用“…到…”格式", "positive": "市场到线索、线索到回款、概念到市场", "negative": ""},
            {"rule": "使能/支持类流程命名", "desc": "针对使能类和支持类流程：可用名称+动词格式", "positive": "服务交付、采购、管理人力资源、管理财经", "negative": ""},
        ],
        "not_examples": [
            "具体的流程组或业务流程（低层级流程元素，如“管理线索”是L2不是L1）",
            "部门/职能名称（如“财务部”“销售部”）",
            "单个业务活动或操作",
        ],
        "definition": [],
    ...
    "流程组（L2）": {
        "description": "在流程类之下的流程分组，一个流程组具有相同的流程价值定位，其内部的业务运作逻辑是相似的、强相关的。",
        "identification": [
            {"rule": "逻辑内聚", "desc": "组内流程具有相似的业务运作逻辑，强相关。", "positive": "L2-管理线索下的收集与生成、验证与分发、跟踪与培育线索等L3流程运作逻辑相似", "negative": "将运作逻辑不相关的流程（如线索管理与合同管理）放入同一流程组"},
            {"rule": "能力对齐", "desc": "每个流程组对应或聚合相关的业务能力", "positive": "L2-管理合同履行聚合合同签订、开票、回款等合同履行相关能力", "negative": ""},
            {"rule": "边界清晰", "desc": "流程组之间边界明确，避免重叠", "positive": "L1-线索到回款下：L2-管理线索、L2-管理机会点、L2-管理合同履行边界互不重叠", "negative": "同一业务能力在两个流程组中重复出现"},
        ],
        "naming": [
            {"rule": "动词+名词", "desc": "通常是动词+名词的形式，行业通用名称可直接采用", "positive": "管理线索、管理机会点、管理合同履行", "negative": ""},
            {"rule": "全局唯一", "desc": "流程组名称全局唯一", "positive": "", "negative": "同一企业内存在两个同名流程组"},
            {"rule": "不用内部职能名称", "desc": "一般不使用内部职能名称", "positive": "管理线索", "negative": "销售部线索管理（使用内部职能名称）"},
        ],
        "not_examples": [
            "L1流程分类（如“线索到回款”）",
            "L3业务流程（如“收集与生成线索”）",
    ...
    "业务流程（L3）": {
        "description": "在特定企业环境及资源保障下，为了实现客户价值和企业商业目标而形成的一套规范业务运作规则和机制：即通过一系列可重复、有逻辑顺序的活动，按照相关的政策和业务规则，将一个或多个输入转换成明确的、可度量的、有价值的输出。",
        "identification": [
            {"rule": "边界由核心业务对象决定", "desc": "L3流程的边界，由其承载的“核心业务对象”的生命周期状态跃迁所决定", "positive": "L3-收集与生成线索的边界由“线索”业务对象从创建到分发的生命周期状态跃迁决定", "negative": "以部门职责或系统模块划分流程边界"},
            {"rule": "一个L3管理一个核心业务对象", "desc": "一个L3只能管理一个核心业务对象。模式一：一对一映射（核心业务对象的全生命周期全部在一个L3）；模式二：一对多映射（核心业务对象的生命周期分布在多个L3）", "positive": "L2-管理线索下：L3-收集与生成线索、L3-验证与分发线索、L3-跟踪与培育线索均围绕“线索”这一核心业务对象的生命周期；L2-管理合同执行下：L3-管理开票", "negative": "一个L3同时管理线索和合同两个核心业务对象"},
        ],
        "naming": [
            {"rule": "动词+名词", "desc": "通常是动词+名词的形式，行业通用名称可直接采用", "positive": "收集与生成线索、验证与分发线索、管理开票", "negative": ""},
            {"rule": "全局唯一", "desc": "业务流程名称全局唯一", "positive": "", "negative": "同一企业内存在两个同名业务流程"},
            {"rule": "不用内部职能名称", "desc": "一般不使用内部职能名称", "positive": "收集与生成线索", "negative": "市场部收集线索（使用内部职能名称）"},
        ],
        "not_examples": [
            "流程组（如“管理线索”是L2不是L3）",
            "管理多个核心业务对象的混合流程",
    ...
    "子流程（L4）": {
        "description": "复杂流程的分解，是一个更大流程的一部分。若流程较复杂或存在多种可交付的重要输出，流程被分解为子流程，使流程相对简明、输出更为单纯，实现职能单元的分工和解耦，便于开发、维护、理解和执行和进一步授权管理。子流程是相对概念，可进一步分解为更小颗粒度的子流程，遵循流程的设计原则。",
        "identification": [
            {"rule": "必要性原则", "desc": "仅在流程较复杂或存在多种可交付的重要输出时分解", "positive": "L3-收集与生成线索较复杂，分解为采集、创建、确认等子流程", "negative": "对本身简单、单一输出的流程进行不必要的分解"},
            {"rule": "MECE原则", "desc": "同一L3下的流程段及场景拆分相互独立，完全穷尽", "positive": "L3-收集与生成线索下：L4-采集线索、L4-创建线索、L4-确认线索相互独立且完全穷尽", "negative": "拆分后的子流程互相重叠或遗漏部分场景"},
        ],
        "naming": [
            {"rule": "动词+名词", "desc": "通常是动词+名词的形式，行业通用名称可直接采用", "positive": "采集线索、创建线索、确认线索", "negative": ""},
            {"rule": "全局唯一", "desc": "子流程名称全局唯一", "positive": "", "negative": "同一企业内存在两个同名子流程"},
            {"rule": "不用内部职能名称", "desc": "一般不使用内部职能名称", "positive": "采集线索", "negative": "市场部采集（使用内部职能名称）"},
        ],
        "not_examples": [
            "简单流程的不必要分解",
            "业务活动（L5，如“校验线索信息”）",
    ...
    "业务活动（L5）": {
        "description": "流程的基本单元，是一组相互联系的、有明确成果和输出的任务或行动。活动从L3业务能力拆解，按场景编排形成L4流程，本身可下挂L6操作指导书。",
        "identification": [
            {"rule": "流程基本单元", "desc": "从L3能力拆解的执行单元，是一组相互联系的、有明确成果和输出的任务或行动", "positive": "L4-确认线索下：L5-校验线索信息、L5-查重比对、L5-标注线索来源各有明确成果和输出", "negative": "无明确成果和输出的随意操作"},
            {"rule": "单一主责角色", "desc": "一个活动一个主责角色，可有多个配合角色；多主责角色须拆分", "positive": "销售订单：录入+财务信用审核→两个活动（不同主责角色，应拆分）", "negative": "法务审核：Agent预审+人工确认属于同一主责，是一个活动两个步骤，不应拆为两个活动"},
            {"rule": "明确输入输出", "desc": "输入有来源，输出有使用者且为独立交付物", "positive": "查重比对的输入是待确认线索，输出是查重结果，供后续确认环节使用", "negative": "输入来源不明或输出无人使用的活动"},
            {"rule": "场景化编排", "desc": "按业务场景编排形成L4端到端流程；复杂活动可下挂L6操作指导书", "positive": "校验线索信息、查重比对、标注线索来源按确认线索场景编排形成L4流程", "negative": ""},
            {"rule": "MECE原则", "desc": "双层应用：同一L3下拆出的活动互斥、穷尽（主）；同一L4编排的活动完整、无冗余（辅）。拆分维度优先级：①责任角色（最优先）②执行系统 ③业务对象", "positive": "同一L3下拆出的活动互斥且穷尽所有场景", "negative": "同一L4下编排的活动互相重叠或遗漏场景"},
            {"rule": "颗粒度管控", "desc": "拆分触发：多主责角色/多独立业务价值/系统边界可分/I/O结构不稳定；合并触发：相同主责+相同I/O/相同价值且高频连续/步骤过于碎片化。总原则：能独立交付价值、能明确责任主体、能指导操作、能支撑IT实现", "positive": "多主责角色的任务拆分为多个活动；相同主责且相同I/O的步骤合并", "negative": "步骤过于碎片化，拆出大量无独立价值的微小活动"},
            {"rule": "独立活动判断", "desc": "不同主责角色/不同业务价值/独立I/O边界/不同规则集/可独立触发→独立活动；同一主责/同一交付物不同阶段/共享I/O/同一规则集→同一活动内步骤", "positive": "不同主责角色的两个任务识别为两个独立活动", "negative": "同一主责、同一交付物不同阶段被误拆为两个活动"},
        ],
        "naming": [
            {"rule": "动宾结构命名", "desc": "以动宾结构命名（动词+名词），语义清晰", "positive": "校验线索信息、查重比对、标注线索来源", "negative": "线索校验流程（带流程概念，不是活动命名）"},
            {"rule": "全局唯一", "desc": "业务活动名称全局唯一", "positive": "", "negative": "同一企业内存在两个同名业务活动"},
            {"rule": "不用内部职能名称", "desc": "一般不使用内部职能名称", "positive": "校验线索信息", "negative": "市场部校验（使用内部职能名称）"},
        ],
        "not_examples": [
            "同一活动内的步骤（同一主责、同一交付物不同阶段、共享I/O、同一规则集）",
            "组织职能/部门职责（如“财务管理职责”）",
            "跨多个主责角色未拆分的端到端流程",
            "系统功能/界面操作（如“点击提交按钮”）",
        ],
...
# 注意：BA关键词需放在DA之前，避免"流程分类（L1）"这类列名被DA的"L1"关键词误匹配
COLUMN_TYPE_KEYWORDS = {
    "流程分类（L1）": ["流程分类", "流程类（L1）", "L1流程"],
    "流程组（L2）": ["流程组", "L2流程"],
    "业务流程（L3）": ["业务流程", "L3流程", "流程名称"],
    "子流程（L4）": ["子流程", "L4流程"],
    "业务活动（L5）": ["业务活动", "流程活动", "活动名称", "L5活动"],
    "主题域分类": ["主题域分类", "分类名称"],
    "主题域分组": ["主题域分组", "分组名称"],
    "主题域": ["主题域名称", "主题域"],
